...
sec("F2 — verified-first now-playing art (the 'no cover in player' battery)");
{
  ...
  // F3 — GPU-sin fence: the player art surfaces must stay OFF the flaky
  // hardware-composite path (the actual device-cause class, found 2026-08-29).
  ok(!/\.artwork-main-img\s*\{[^}]*will-change/.test(HTML), "hero image has NO will-change promotion");
  ok(!/\.ambient-img\s*\{[^}]*will-change/.test(HTML), "ambient images have NO will-change promotion");
  ok(!/@keyframes ambientDrift\s*\{[^}]*filter:/s.test(HTML), "ambientDrift never animates filter: (no per-frame invalidation)");
  ok(/\.ambient-img\.baked\s*\{[^}]*filter:\s*none/.test(HTML), "baked ambient thumbnails skip the CSS blur filter");
  ok(HTML.includes("function bakeAmbientThumb(") && HTML.includes("ambientThumbCache"),
    "ambient pixel-bake pipeline present (blur baked once, not per-frame)");
  ok(HTML.includes("armPlayerArtDiagnostics"), "in-app hero diagnostics armed (device truth-table)");
}
